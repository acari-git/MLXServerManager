...
## v6.13.0 Completed: Signed Zip Dry-Run Checklist

- Add `docs/signed_zip_dry_run_checklist.md` as a docs-only checklist for future signed zip dry-run work.
- Define dry-run scope, repository state checks, release scope checks, build path planning, signing placeholder planning, zip naming, forbidden entries, release notes structure, fallback decisions, and a local dry-run log template.
- Keep the current downloadable app binary at `v6.5.1`; no new app zip is produced.
- Keep Direct Mode unchanged; no signing, notarization, stapling, DMG, installer, auto-update, release automation, runtime behavior change, telemetry, background monitoring, model download, model deletion, model scanning, or cache cleanup.
...
## v6.14.0 Completed: Signed Zip Local Dry-Run Execution Notes

- Add `docs/signed_zip_local_dry_run_execution_notes.md` as docs-only execution notes for future local-only signed zip dry runs.
- Define dry-run execution boundaries, suggested local records, scope checks, public asset checks, placeholder status wording, stop conditions, evidence handling, public documentation criteria, and fallback policy.
- Keep the current downloadable app binary at `v6.5.1`; no new app zip is produced.
- Keep Direct Mode unchanged; no signing, notarization, stapling, DMG, installer, auto-update, release automation, runtime behavior change, telemetry, background monitoring, model download, model deletion, model scanning, or cache cleanup.

## v6.14.1 Completed: Signed Zip Local Dry-Run Execution Notes Polish

- Add scrub checklist guidance to `docs/signed_zip_local_dry_run_execution_notes.md`.
- Add conservative public summary wording for docs-only local dry-run releases.
- Add handoff criteria before moving from local dry-run notes to real signed zip implementation.
- Keep the current downloadable app binary at `v6.5.1`; no new app zip is produced.
- Keep Direct Mode unchanged; no signing, notarization, stapling, DMG, installer, auto-update, release automation, runtime behavior change, telemetry, background monitoring, model download, model deletion, model scanning, or cache cleanup.

## v6.15.0 Completed: Signed Zip Implementation Readiness Closeout

- Add `docs/signed_zip_implementation_readiness_closeout.md` as a docs-only closeout for signed zip readiness work.
- Map the signed distribution, notarization, implementation readiness, local signing draft, dry-run checklist, and local dry-run execution notes documents.
- Define signed zip implementation entry criteria, no-go criteria, candidate next releases, public wording policy, asset naming policy, and first signed zip verification expectations.
- Keep the current downloadable app binary at `v6.5.1`; no new app zip is produced.
- Keep Direct Mode unchanged; no signing, notarization, stapling, DMG, installer, auto-update, release automation, runtime behavior change, telemetry, background monitoring, model download, model deletion, model scanning, or cache cleanup.
...
## v6.23.0 Completed: Diagnostics Result Fixture Files

- Add initial diagnostics fixture data under `MLXServerManagerTests/Fixtures/Diagnostics/`.
- Add result fixtures for pass, warning, fail, skipped, unknown, cancelled, and timeout states.
- Add redaction, aggregation, negative, and copied-summary fixtures.
- Keep the current downloadable app binary at `v6.5.1`; no new app zip is produced.
- Keep Direct Mode unchanged; no Swift source changes, tests, diagnostics execution, endpoint testing, inference requests, background monitoring, traffic inspection, telemetry, diagnostics history persistence, runtime behavior change, or release automation.
