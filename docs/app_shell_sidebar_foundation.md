...
The implemented shell remains limited to app shell/sidebar foundation and does not pull in Profiles, Inspector, Logs, Client Setup, Metrics, or behavior changes. v6.0.1 keeps that boundary while adding small sidebar polish. v6.0.2 keeps accessibility ownership inside the sidebar row and records the v6.0.x shell state before v6.1.0 begins.
