...
v4.1.0 adds Dashboard UI Refresh design guidance. v4.2.0 adds a small dashboard foundation near first-run guidance so Current Target and Server State are easier to read. v4.3.0 polishes Current Target wording so first-time users can better distinguish no target, managed server, external server, adopted external server, and unavailable endpoint states. v4.4.0 polishes Server State wording so first-time users can better distinguish managed process state, readiness, lifecycle expectations, external connection context, and failed or unavailable states. v4.5.0 adds display-only guidance for logs, diagnostics, readiness failures, port busy states, unavailable targets, and external server log boundaries. v4.6.0 adds display-only guidance for selected profile metadata, profile endpoint, current target relationship, and metadata-only Import / Export safety. v4.7.0 adds display-only Next Steps guidance for first-run setup, managed Start, external adoption, readiness expectations, Direct Mode, and manual troubleshooting boundaries. v4.8.0 adds display-only grouping headings and clearer Dashboard scan order so Next Steps, target state, troubleshooting, and profile context are easier to distinguish. v4.9.0 adds display-only Client Setup guidance so first-time users can identify active endpoint, selected profile model ID, readiness state, and Direct Mode copy context before configuring an OpenAI-compatible client. v5.0.0 finalizes Dashboard UI Refresh v1 as the stable display-oriented overview for first-run and returning-user context. v5.1.0 keeps that surface stable and documents future larger layout work separately. v5.2.0 documents a possible future Full App Layout Refresh without changing first-run onboarding behavior. Future dashboard work should keep first-run guidance short and state-aware, place it near setup status when useful, and avoid turning onboarding into hidden automation, model download, server start, external adoption, client configuration automation, or inference routing.
