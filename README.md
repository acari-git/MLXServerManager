...
Diagnostics Result Fixture Design is documented in [docs/diagnostics_result_fixture_design.md](docs/diagnostics_result_fixture_design.md). v6.21.0 is a docs-only design release for future diagnostics result fixtures. It defines fixture naming policy, required status, severity, scope, category, redaction, copy summary, timeout/cancellation, external server ownership, selected profile mutation, aggregation, and fixture data safety expectations without adding tests, running diagnostics, or changing runtime behavior.
