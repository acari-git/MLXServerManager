...
Client Setup Surface design is documented in [docs/client_setup_surface.md](docs/client_setup_surface.md). v6.4.0 implements the first Client Setup surface as a top-level sidebar destination for copy-safe OpenAI-compatible setup values. It does not implement generated client configs, API key storage, endpoint testing, proxying, or behavior changes.
