...
```text
MLXServerManager-v14.0.0-unsigned.zip
```
...
`v14.0.0` is the latest app-code release. Recent v6/v7/v8/v9/v10/v11/v12/v13 staged commits do not replace this app binary.

Verify the checksum before opening the app:

```text
SHA-256: ff78c687c53d31bac89db4344ea4b099dfb72d1ef5d1901f140b6eaa6758e826
```

From the folder containing the downloaded zip, you can check it with:

```sh
shasum -a 256 MLXServerManager-v14.0.0-unsigned.zip
```
...
Do not download the source archive if you want the app binary. Use the named `MLXServerManager-v14.0.0-unsigned.zip` release asset.
...
v15.0.0 focuses on GUI workflow integration. Dashboard is now an overview and launcher; Models owns profile management; Downloads owns acquisition and recovery; Runtime owns diagnostics and benchmark; Settings owns app settings and language; Logs owns troubleshooting context. Japanese / English language switching remains available from Settings while preserving Direct Mode.
