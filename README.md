...
Model Download Design is documented in [docs/model_download_design.md](docs/model_download_design.md). v6.17.0 is a docs-only design release for possible future model download support. It defines download boundaries, destination path policy, credential handling boundaries, download task boundaries, compatibility wording, profile integration boundaries, safety checklist, and verification expectations without downloading models, deleting models, scanning model directories, cleaning caches, persisting tokens, or changing runtime behavior.
